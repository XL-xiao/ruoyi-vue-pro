...
(
    1
) NOT NULL DEFAULT 0 COMMENT '是否删除',
    PRIMARY KEY
(
    `id`
)
...
(
    128
) DEFAULT NULL COMMENT '卖点',
    `description` text COMMENT '描述',
    `pic_urls` varchar
(
    1024
) DEFAULT '' COMMENT '商品轮播图地址数组，以逗号分隔最多上传15张',
    `video_url` varchar
(
    128
) DEFAULT NULL COMMENT '商品视频',
...
(
    1
) DEFAULT NULL COMMENT '上下架状态： 0 上架（开启） 1 下架（禁用）-1 回收',
...
(
    128
) NOT NULL COMMENT '品牌名称',
    `pic_url` varchar DEFAULT NULL COMMENT '品牌图片',
    `sort` int NOT NULL DEFAULT '0' COMMENT '排序字段',
    `description` varchar
(
    256
) NOT NULL DEFAULT '0' COMMENT '品牌描述',
    `status` bit
(
    1
) DEFAULT NULL COMMENT '状态',
